...
//----------------------------------------------------------------------------------------------------------------------        
    ...
    private string OpenFilePanel() {
        string path = EditorUtility.OpenFilePanel(addDCCToolLabel, m_lastOpenedFolder, "");

        if (!string.IsNullOrEmpty(path)) m_lastOpenedFolder = Path.GetDirectoryName(path);

        return path;
    }

    private string OpenFolderPanel() {
        string path = EditorUtility.OpenFolderPanel(addDCCToolLabel, m_lastOpenedFolder, "");

        if (!string.IsNullOrEmpty(path)) m_lastOpenedFolder = path;

        return path;
    }

    private void OnAddDCCToolButtonClicked(EventBase evt) {
        string path = null;
    ...
    private void OnAutoDetectDCCButtonClicked() {
        MeshSyncEditorSettings settings = MeshSyncEditorSettings.GetOrCreateSettings();
        if (settings.AddInstalledDCCTools()) SetupInternal(m_root);
    }
    ...
    private void OnLaunchDCCToolButtonClicked(EventBase evt) {
        DCCToolInfo dccToolInfo = GetEventButtonUserDataAs<DCCToolInfo>(evt.target);
    ...
    private void OnInstallPluginButtonClicked(EventBase evt) {
...
//----------------------------------------------------------------------------------------------------------------------        
    ...
    private void OnCheckPluginUpdatesButtonClicked() {
        ...
        foreach (Button installPluginButton in m_installPluginButtons) installPluginButton.SetEnabled(false);
        ...
        PackageRequestJobManager.CreateSearchRequest("com.unity.meshsync.dcc-plugins", /*offline=*/ false, (packageInfo) => {
            ...
            foreach (KeyValuePair<string, Label> kv in m_dccStatusLabels) UpdateDCCPluginStatus(kv.Value);

            m_updateFooterStatusFinished = true;
        }, (req) => { m_updateFooterStatusFinished = true; });
    }

    private void UpdateLatestCompatibleDCCPlugin(VersionsInfo versionsInfo) {
    ...
    private IEnumerator UpdateFooterStatusLabel(string reqStatusText, System.Action onFinished) {
        ...
        if (mainStatusText.Length >= MAX_PREFIX_LENGTH) mainStatusText = reqStatusText.Substring(0, MAX_PREFIX_LENGTH);

        string labelText = mainStatusText;
        while (!m_updateFooterStatusFinished) {
            labelText += ".";
            if (labelText.Length > MAX_STATUS_LENGTH) labelText = mainStatusText;
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------        
    private void UpdateDCCPluginStatus(Label statusLabel) {
    ...
    private void UpdateDCCPluginStatusLabel(Label statusLabel, DCCToolInfo dccToolInfo, DCCPluginStatus status, string installedPluginVersionStr) {
        PackageVersion pluginVer = MeshSyncEditorConstants.GetPluginVersion();
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    private static T GetEventButtonUserDataAs<T>(IEventHandler eventTarget) where T : class {
        Button button = eventTarget as Button;
        if (null == button) return null;
