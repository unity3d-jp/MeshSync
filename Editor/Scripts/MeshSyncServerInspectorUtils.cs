...
internal static class MeshSyncServerInspectorUtils {
    private static HashSet<string> foldouts = new HashSet<string>();

    private const int MAX_SLIDER_RANGE = 1000;
    ...
    public static void DrawSliderForProperties(List<PropertyInfoDataWrapper> props,
        ...
        string lastPathDrawn = string.Empty;
        bool   expanded      = false;

        foreach (PropertyInfoDataWrapper prop in props) {
            if (parentNameFilters != null && Array.IndexOf(parentNameFilters, prop.path) == -1) continue;
            ...
            if (prop.path != lastPathDrawn) {
                ...
                if (expanded && allEnabled != null && allEnabledChanged != null) {
                    bool wasAllEnabled = allEnabled(lastPathDrawn);

                    bool newAllEnabled = EditorGUILayout.ToggleLeft($"All properties in {lastPathDrawn}", wasAllEnabled);
                    if (newAllEnabled != wasAllEnabled) allEnabledChanged(lastPathDrawn, newAllEnabled);
                }
            }
    ...
    private static object DrawArrayFields<T>(PropertyInfoDataWrapper prop,
        ...
        switch (a.Length) {
            ...
            default: {
                ...
                for (int i = 0; i < prop.arrayLength; i++) {
                    ...
                    if (EditorGUI.EndChangeCheck()) {
                        T[] newValueAsArray = new T[prop.arrayLength];
    ...
    public static void OpenDCCAsset(MeshSyncServer server) {
        RunMode previousRunMode = server.m_DCCInterop != null ? server.m_DCCInterop.runMode : RunMode.GUI;

        Object asset = server.DCCAsset;
