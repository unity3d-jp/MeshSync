...
//----------------------------------------------------------------------------------------------------------------------
    ...
    public override Playable CreatePlayable(PlayableGraph graph, GameObject go) {
        ...
        if (!m_sceneCachePlayer) return ScriptPlayable<SceneCachePlayableBehaviour>.Create(graph, behaviour);
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    private static void SetClipCurveInEditor(TimelineClip clip, AnimationCurve animationCurveToApply) {
        ...
        if (!shouldRefresh) {
            AnimationCurve shownCurve = AnimationUtility.GetEditorCurve(clip.curves, GetTimeCurveBinding());
        ...
        AnimationUtility.SetEditorCurve(clip.curves, GetTimeCurveBinding(), animationCurveToApply);

        if (shouldRefresh) TimelineEditor.Refresh(RefreshReason.ContentsAddedOrRemoved);
    }

    private float FindSceneCacheDurationInEditor(TimelineClip clip) {
        ISceneCacheInfo sceneCacheInfo = m_sceneCachePlayer.ExtractSceneCacheInfo(true);
        if (null == sceneCacheInfo) return (float)clip.duration;
    ...
    private static bool CurveApproximately(AnimationCurve x, AnimationCurve y) {
        if (null == x && null == y)
            return true;
    ...
    private static bool KeyframeApproximately(Keyframe k0, Keyframe k1) {
...
//----------------------------------------------------------------------------------------------------------------------    
    ...
    internal ExposedReference<SceneCachePlayer> GetSceneCachePlayerRef() {
        return m_sceneCachePlayerRef;
    }
    ...
    internal LimitedAnimationController GetOverrideLimitedAnimationController() {
        return m_overrideLimitedAnimationController;
    }
    ...
    internal AnimationCurve GetAnimationCurve() {
        return m_animationCurve;
    }


    internal SceneCachePlayableAssetEditorConfig GetEditorConfig() {
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    internal void OnClipChanged() {
        ...
        ISceneCacheInfo sceneCacheInfo = m_sceneCachePlayer.ExtractSceneCacheInfo(true);
        if (null == sceneCacheInfo) return;

        m_animationCurve = RecreateClipCurveInEditor(track, clip, clipData, sceneCacheInfo);
    }

    private static void AddCurveKey(List<Keyframe> keys, float time, int frameToPlay, int numFrames, KeyFrameMode mode) {
    ...
    private static AnimationCurve RecreateClipCurveInEditor(TrackAsset track, TimelineClip clip, SceneCacheClipData clipData, ISceneCacheInfo sceneCacheInfo) {
    ...
    internal static EditorCurveBinding GetTimeCurveBinding() {
        return m_timeCurveBinding;
    }
