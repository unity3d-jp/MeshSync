...
//----------------------------------------------------------------------------------------------------------------------
    internal override void DestroyV() {
        foreach (PlayableKeyFrame keyFrame in m_keyFrames) keyFrame.Destroy();
    }
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    internal void RequestKeyFrameMarkers(bool req, bool forceShow = false) {
        ...
        if (UpdateFrameMarkersVisibility()) RefreshKeyFrames();
    }

    internal int GetNumKeyFrames() {
...
//----------------------------------------------------------------------------------------------------------------------
    private void DestroyPlayableFrames() {
        ...
        foreach (PlayableKeyFrame frame in m_keyFrames) frame?.Destroy();
    }
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    internal void OnGraphStart() {
        ...
        if (m_needToRefreshTimelineEditor) {
            foreach (PlayableKeyFrame playableKeyFrame in m_keyFrames) playableKeyFrame.RefreshMarker(m_keyFrameMarkersVisibility);
    ...
    internal void OnClipChanged() {
        ...
        foreach (int moveDestIndex in movedKeyFrames.Keys) keyFramesToDisable.Remove(moveDestIndex);
        ...
        foreach (int i in keyFramesToDisable) m_keyFrames[i].SetEnabled(false);

        if (movedKeyFrames.Count > 0) m_needToRefreshTimelineEditor = true;
    }
    ...
    internal void ResetKeyFrames() {
        ...
        InitKeyFrames(0, numIdealNumPlayableFrames - 1, 1, KeyFrameMode.Continuous);
    }
    ...
    private bool NeedsRefreshKeyFrames() {
        TimelineClip clip = GetOwnerIfReady();
        if (null == clip) return false;

        if (!Mathf.Approximately(m_lastClipStartTimeOnRefresh, (float)clip.start)) return true;

        if (!Mathf.Approximately(m_lastClipDurationOnRefresh, (float)clip.duration)) return true;

        if (!Mathf.Approximately(m_lastClipTimeScaleOnRefresh, (float)clip.timeScale)) return true;

        if (!Mathf.Approximately(m_lastClipInOnRefresh, (float)clip.clipIn)) return true;
    ...
    private void RefreshKeyFrames() {
        ...
        if (numIdealNumPlayableFrames != prevNumPlayableFrames) {
            ...
            if (prevNumPlayableFrames > 0) {
                ...
                for (int i = 0; i < minNumPlayableFrames; ++i) m_keyFrames[i].SetKeyFrameMode(prevKeyFrameModes[i]);
            }
        }
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    private void UpdateKeyFramesSize(int reqPlayableFramesSize) {
        ...
        if (m_keyFrames.Count < reqPlayableFramesSize) {
            ...
            for (int i = m_keyFrames.Count; i < reqPlayableFramesSize; ++i) newPlayableFrames.Add(CreateKeyFrame(this, i, timePerFrame));
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    [CanBeNull]
    private static PlayableKeyFrame FindEnabledKeyFrame(IList<PlayableKeyFrame> keyFrames, int startIndex, int endIndex) {
...
#if UNITY_EDITOR
    private double m_timelineWidthPerFrame = short.MaxValue;
