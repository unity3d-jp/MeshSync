...
//----------------------------------------------------------------------------------------------------------------------    
    ...
    internal MeshSyncPlayerConfig(MeshSyncPlayerConfig other) {
        ...
        m_componentSyncSettings = new List<ComponentSyncSettings>();
        for (int i = 0; i < SYNC_COUNT; ++i) m_componentSyncSettings.Add(new ComponentSyncSettings(other.m_componentSyncSettings[i]));
...
//----------------------------------------------------------------------------------------------------------------------    
    ...
    public void OnAfterDeserialize() {
        //Validate
        if (null == m_componentSyncSettings || m_componentSyncSettings.Count != SYNC_COUNT) InitComponentSyncSettings();
...
//----------------------------------------------------------------------------------------------------------------------    
    internal ModelImporterSettings GetModelImporterSettings() {
    ...
    internal ComponentSyncSettings GetComponentSyncSettings(int index) {
        return m_componentSyncSettings[index];
    }

    internal bool IsPhysicalCameraParamsUsed() {
        return m_usePhysicalCameraParams;
    }

    internal void UsePhysicalCameraParams(bool use) {
        m_usePhysicalCameraParams = use;
    }
...
//----------------------------------------------------------------------------------------------------------------------    
    ...
    public  bool OverwriteMaterials = false;
...
#pragma warning restore 414
    ...
    private enum MeshSyncPlayerConfigVersion {
        NO_VERSIONING = 0,
