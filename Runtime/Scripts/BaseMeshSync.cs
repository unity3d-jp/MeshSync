...
namespace Unity.MeshSync
{
    ...
    public abstract partial class BaseMeshSync : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        public string GetAssetsFolder() { return m_assetsFolder; }
...
#pragma warning restore 612
        ...
        EntityRecord UpdateInstancedEntity(TransformData data)
        {
            ...
            if (!this.m_clientInstancedEntities.TryGetValue(data.path, out EntityRecord rec) || rec.go == null)
            {
                var trans =
        ...
        internal bool EraseInstanceInfoRecord(Identifier identifier)
        {
            ...
            if (this.m_clientInstances.TryGetValue(path, out InstanceInfoRecord rec))
            {
                ...
                if (rec.renderer != null)
                {
                    DestroyImmediate(rec.renderer);
                }

                foreach (var obj in rec.instanceObjects)
                {
                    DestroyImmediate(obj);
                }
