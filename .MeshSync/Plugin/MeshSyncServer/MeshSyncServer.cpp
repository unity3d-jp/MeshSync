...
float CalculateLinearTangent(IArray<Keyframe>& curve, int i1, int i2)
{
...
inline TangentMode GetKeyLeftTangentMode(const Keyframe& key)
{
    return static_cast<TangentMode>((key.tangent_mode & kLeftTangentMask) >> 1);
}

inline TangentMode GetKeyRightTangentMode(const Keyframe& key)
{
    return static_cast<TangentMode>((key.tangent_mode & kRightTangentMask) >> 5);
}

inline float SafeDeltaDivide(float y, float x)
{
...
inline void RecalculateAutoKey(IArray<Keyframe>& curve, int key, float bias)
{
    if (curve.size() < 2)
        return;
    ...
    if (key == 0)
    {
        curve[key].in_tangent = curve[key].out_tangent = 0;

        if (curve.size() > 1)
            curve[key].in_weight = curve[key].out_weight = kDefaultWeight;
    }
    ...
    else
    {
        ...
        if ((m1 > 0 && m2 > 0) || (m1 < 0 && m2 < 0))
        {
            ...
            {
                float b = SafeDeltaDivide(dy1 - upper_dy, lower_dy);
                //T mp = b * m2 + (1.0F - b) * m;
                float mp = (1.0F - b) * m;

                curve[key].in_tangent = mp; curve[key].out_tangent = mp;
            }
            else if (std::abs(dy1) < std::abs(lower_dy))
            {
                float b = SafeDeltaDivide(dy1, lower_dy);
                //T mp = b * m + (1.0F - b) * m1;
                float mp = b * m;
            ...
            {
                curve[key].in_tangent = m; curve[key].out_tangent = m;
            }
        }
        else
        {
            curve[key].in_tangent = curve[key].out_tangent = 0.0f;
        }

        curve[key].in_weight = kDefaultWeight;
        curve[key].out_weight = kDefaultWeight;
    }
}
...
inline void UpdateTangents(IArray<Keyframe>& curve, int index)
{
    ...
    // linear
    if (GetKeyLeftTangentMode(key) == kTangentModeLinear && index >= 1)
        key.in_tangent = CalculateLinearTangent(curve, index, index - 1);
    if (GetKeyRightTangentMode(key) == kTangentModeLinear && index + 1 < curve.size())
        key.out_tangent = CalculateLinearTangent(curve, index, index + 1);

    // smooth
    if (GetKeyLeftTangentMode(key) == kTangentModeClampedAuto || GetKeyRightTangentMode(key) == kTangentModeClampedAuto)
        RecalculateAutoKey(curve, index, 0.5f);

    // constant
    if (GetKeyLeftTangentMode(key) == kTangentModeConstant)
        key.in_tangent = std::numeric_limits<float>::infinity();
    if (GetKeyRightTangentMode(key) == kTangentModeConstant)
        key.out_tangent = std::numeric_limits<float>::infinity();
}

static void msSetTangentMode(Keyframe *key, int n, InterpolationType it)
{
...
static inline void FillCurve(const RawVector<ms::TVP<bool>>& src, Keyframe *x, InterpolationType it)
{
    int n = (int)src.size();
    for (int i = 0; i < n; ++i) {
        const auto v = src[i];
...
static inline void FillCurve(const RawVector<ms::TVP<float>>& src, Keyframe *x, InterpolationType it)
{
    int n = (int)src.size();
    for (int i = 0; i < n; ++i) {
        const auto v = src[i];
...
static inline void FillCurves(const RawVector<ms::TVP<float3>>& src, Keyframe *x, Keyframe *y, Keyframe *z, InterpolationType it)
{
    int n = (int)src.size();
    for (int i = 0; i < n; ++i) {
        const auto v = src[i];
...
static inline void FillCurves(const RawVector<ms::TVP<float4>>& src, Keyframe *x, Keyframe *y, Keyframe *z, Keyframe *w, InterpolationType it)
{
    int n = (int)src.size();
    for (int i = 0; i < n; ++i) {
        const auto v = src[i];
...
static inline void FillCurves(const RawVector<ms::TVP<quatf>>& src, Keyframe *x, Keyframe *y, Keyframe *z, Keyframe *w, InterpolationType it)
{
    int n = (int)src.size();
    for (int i = 0; i < n; ++i) {
        const auto v = src[i];
...
msAPI void      msTransformAFillTranslation(ms::TransformAnimation *self, Keyframe *x, Keyframe *y, Keyframe *z, InterpolationType it)
{
...
msAPI void      msTransformAFillScale(ms::TransformAnimation *self, Keyframe *x, Keyframe *y, Keyframe *z, InterpolationType it)
{
...
msAPI void      msTransformAFillVisible(ms::TransformAnimation *self, Keyframe *v, InterpolationType it)
{
...
msAPI void      msCameraAFillFov(ms::CameraAnimation *self, Keyframe *v, InterpolationType it)
{
...
msAPI void      msCameraAFillNear(ms::CameraAnimation *self, Keyframe *v, InterpolationType it)
{
...
msAPI void      msCameraAFillFar(ms::CameraAnimation *self, Keyframe *v, InterpolationType it)
{
...
msAPI void      msCameraAFillHAperture(ms::CameraAnimation *self, Keyframe *v, InterpolationType it)
{
...
msAPI void      msCameraAFillVAperture(ms::CameraAnimation *self, Keyframe *v, InterpolationType it)
{
...
msAPI void      msCameraAFillFocalLength(ms::CameraAnimation *self, Keyframe *v, InterpolationType it)
{
...
msAPI void      msCameraAFillFocusDistance(ms::CameraAnimation *self, Keyframe *v, InterpolationType it)
{
...
msAPI void      msLightAFillColor(ms::LightAnimation *self, Keyframe *x, Keyframe *y, Keyframe *z, Keyframe *w, InterpolationType it)
{
...
msAPI void      msLightAFillIntensity(ms::LightAnimation *self, Keyframe *v, InterpolationType it)
{
...
msAPI void      msLightAFillRange(ms::LightAnimation *self, Keyframe *v, InterpolationType it)
{
...
msAPI void      msLightAFillSpotAngle(ms::LightAnimation *self, Keyframe *v, InterpolationType it)
{
...
msAPI void          msMeshAFillBlendshapeWeight(ms::MeshAnimation *self, int bi, Keyframe *v, InterpolationType it)
{
...
msAPI void  msPointsAFillTime(ms::PointsAnimation *self, Keyframe *v, InterpolationType it)
{
